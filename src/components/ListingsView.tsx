...
export function ListingsView() {
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings.filter((l) => {
      const haystack = `${l.listing?.title ?? ''} ${l.listing?.address ?? ''}`.toLowerCase();
      ...
      {isInitializing ? (
        ...
        <>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Inserate durchsuchen (Titel, Adresse)…"
                  className="w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2.5 pl-9 pr-9 text-sm placeholder:text-gray-400 focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
                {search && (
                  <button
                    onClick={() => setSearch('')}
              ...
              <p className="mt-2 text-xs text-gray-400">
                {search ? (
                  <>
                    {filteredListings.length} von {listings.length} Inserat
                    {listings.length !== 1 ? 'e' : ''} gefunden
                  </>
                ) : (
                  <>
          ...
          {listings.length > 0 && filteredListings.length === 0 && (
            <div className="text-center py-16 text-gray-400">
              <div className="text-4xl mb-3">🔍</div>
              <p className="text-sm">Keine Inserate gefunden für &quot;{search}&quot;</p>
            </div>
          )}
        </>
      )}
